import { CANCEL_CONFIRMATION, CHANGE_CONVERSATIONS_SEARCH_TERM, CHANGE_PINS_SEARCH_TERM, CHANGE_PINS_SORT_CRITERIA, CHANGE_USER_PINS_SEARCH_TERM, CHANGE_USER_PINS_SORT_CRITERIA, CLOSE_AUTHORIZATION_WINDOW, CONFIRM_ACTION, CREATE_PIN, DELETE_PIN, EDIT_PIN, FORCE_UPDATE, HIDE_MESSAGE, LOGIN, LOGOUT, OPEN_LOGIN_WINDOW, OPEN_SIGN_UP_WINDOW, RESET_SHOWED_PINS_COUNT, SAVE_PIN, SET_CONFIRMATION_VALUES, SHOW_MESSAGE, SHOW_MORE_PINS, SIGN_UP, UPDATE_PINS_LIST, UPDATE_TAGS_VIEW_FREQUENCY_HISTOGRAM } from "./action_types";

export function updatePinsList(payload){
    return({ type: UPDATE_PINS_LIST, payload });
}

export function showMorePins(){
    return { type: SHOW_MORE_PINS };
}

export function resetShowedPinsCount(){
    return { type: RESET_SHOWED_PINS_COUNT };
}
